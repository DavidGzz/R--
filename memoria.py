class Memoria:
    ...
    def add_int(this, tamanio):
        # this.int[0] = dirección base, this.int[1] es el contador
        if(this.int[0] + this.int[1]) < this.float[0]:
            this.int[1] = this.int[1] + tamanio
            return(this.int[0] + this.int[1] - tamanio)
        else:
    ...
    def add_bool(this, cont):
        if(this.bool[0] + this.bool[1]) < this.limite:
            this.bool[1] += 1
            return(this.bool[0] + this.bool[1] - 1) # Se le quita 1 para que inicie en 0
        else:
            print("Se excedió del número de variables")
    ...
    def add_tipo(this, tipo, tamanioArreglo):
        if tipo == 'char':
            ...
            return this.add_int(int(tamanioArreglo))